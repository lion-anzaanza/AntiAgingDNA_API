...
| **B. 코드 수정 필요** | 코드/스펙이 FE 요구와 다르거나, 정보 자체가 스펙에 없음. **명확한 관례·표준·기존 패턴이 있으면 여기 속한다** | 이 저장소에서 결정하고 구현 (§1.5, §3) |
| **C. 백엔드 단독 결정 불가** | **근거로 삼을 관례·표준·기존 코드 패턴이 전혀 없어 숫자·규칙을 지어내야만 하는 경우만.** 기획·디자인 산출물(문항 구성, 커머스 정책 등)이 선행돼야 하는 것도 포함 | 구현하지 않음. `docs/PLANNING_OPEN_ITEMS.md` 에 항목을 추가하거나 기존 항목과 연결하고, FE 회신에는 "결정 대기"로 명시 |
...
## 1.5. B 와 C 를 가르는 기준 — 창의적 판단 금지, 그러나 미루지도 않는다

C 로 도망치는 것도, 근거 없이 지어내는 것도 둘 다 실패다. 둘을 가르는 질문은 하나뿐이다:
**"이걸 결정할 근거(관례·표준·기존 코드 패턴·화면에 이미 적힌 placeholder)가 있는가?"**

- **있다 → B. 그 자리에서 결정하고 구현한다.** 백엔드 개발자로서 책임지고 처리하는 항목이다.
  "이건 FE/기획 확인이 필요할 것 같다"며 미루지 않는다. 예:
  - 화면 placeholder 가 이미 규칙을 말하고 있다 (예: "8자리 이상, 영문 숫자 포함" → 그 문구 그대로
    `@Pattern` 을 건다. FE 에게 "이 규칙이 맞는지" 되묻지 않는다)
  - RFC/업계 표준이 있다 (에러 응답 포맷, 시각 포맷 등)
  - 기획 문서에 이미 확정된 방향이 있다 (`docs/PLANNING_OPEN_ITEMS.md` 의 "결정 완료"/"확정" 항목)
  - REST·이 코드베이스의 기존 패턴을 그대로 따르면 되는 것 (필드명 컨벤션, 상태 코드 선택 등)
  - 잠정 규칙이 필요하면(예: 아직 화면이 없는 필드의 형식) 업계에서 흔히 쓰는 무난한 기본값으로
    정하고 "잠정" 이라고 명시한 뒤 구현까지 끝낸다 — "정해주세요" 로 공을 넘기지 않는다
- **없다 → C.** 숫자를 하나라도 지어내야 하면(점수 등급 경계값, 채점식 도메인 변경처럼 잘못 지으면
  실제 사용자 점수가 왜곡되는 것) 절대 임의로 채우지 않는다. 이 프로젝트는 `PLANNING_OPEN_ITEMS.md`
  A-1~A-5 에서 이미 "근거 없으면 채점 제외" 원칙을 세워뒀다 — 그 원칙을 그대로 따른다.

기준: **비용(틀렸을 때 되돌리기 어려움) × 근거 부재 정도.** 필드명 하나 잘못 짓는 것과 전 사용자
점수 산식을 잘못 바꾸는 것은 위험도가 다르다 — 후자만 C 로 남긴다.
...
## 4. FE에게 보낼 회신 — 통보다, 합의 요청이 아니다

B 항목(§1.5)은 백엔드가 이미 결정하고 구현을 끝낸 상태로 회신한다. **"이렇게 해도 될까요?" 가
아니라 "이렇게 했습니다, 맞춰주세요"** 다. FE 의견을 구하는 건 진짜 C 항목(근거 없어 못 정한 것)
뿐이다 — B 항목까지 FE 승인을 기다리면 처리한 의미가 없다(백로그가 쌓인 이유 자체가 "물어봤는데
답이 안 왔다"였다).

이 저장소는 기본적으로 `AntiAgingDNA_front` 저장소에 쓰기 권한이 없다고 가정한다. 백로그 문서를
직접 고치지 않는다. 대신:

1. 처리한 항목을 번호 순서(P1부터)로 정리한 **회신 텍스트**를 작성한다. 항목마다: 결론 1줄(B 는
   확정 통보, C 는 "결정 대기" + 이유) + (필요하면) 정확한 필드명·타입·enum 값 + 관련 커밋 해시.
2. 사용자에게 회신 초안을 보여주고 전달 방법을 확인한다 (내용 승인이 아니라 **전달 채널** 확인 —
   PR vs 텍스트):
...
- [ ] C 로 분류한 항목마다 "정말 근거가 없는가"를 §1.5 기준으로 다시 물었는가 — 그냥 애매해서
      미룬 건 없는가

 ...
 *
 * <p>쓰기 경로마다 {@link ScoringService#recalculate(String, LocalDate)} 를 <b>같은
...
public class DiaryService {
    ...
    public Diary save(String userId, LocalDate date, DiaryRequest request) {
        ...
        // recalculate 는 일지를 다시 조회한다. 같은 트랜잭션이고 JPA 가 쿼리 전에 flush 하므로
        // 방금 쓴 내용이 반영된 상태로 채점된다.
        scoringService.recalculate(userId, date);
        return saved;
    }
